...
const CommandPalette: React.FC<CommandPaletteProps> = ({ isOpen, onClose }) => {
  ...
  const fuzzySearch = useCallback((query: string, data: SearchResult[]) => {
    ...
    return data.filter(item => {
      const searchableText = `${item.title} ${item.subtitle} ${item.description} ${item.metadata}`.toLowerCase();
  ...
  // Quick actions
  const quickActions = [
    { icon: Users, label: 'New Customer', action: '/customers/new', color: 'blue' },
    { icon: Target, label: 'New Lead', action: '/leads/new', color: 'purple' },
    { icon: DollarSign, label: 'New Deal', action: '/deals/new', color: 'green' },
    { icon: FileText, label: 'New Invoice', action: '/invoices/new', color: 'orange' },
  ];
                ...
                return (
                  ...
                  >
                    <div className={`p-2 rounded-lg ${
                      ...
                      result.type === 'lead' ? 'bg-purple-100' :
                      result.type === 'deal' ? 'bg-green-100' :
                      result.type === 'invoice' ? 'bg-orange-100' :
                      result.type === 'location' ? 'bg-pink-100' :
                      'bg-gray-100'
                    }`}>
                      <Icon className={`h-5 w-5 ${
                        result.type === 'customer' ? 'text-blue-600' :
                        result.type === 'lead' ? 'text-purple-600' :
                        result.type === 'deal' ? 'text-green-600' :
                        result.type === 'invoice' ? 'text-orange-600' :
                        result.type === 'location' ? 'text-pink-600' :
                        'text-gray-600'
                      }`} />
                    </div>
                    ...
                    <div className="flex-1 text-left">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded-full ${
                          ...
                          result.type === 'deal' ? 'bg-green-100 text-green-700' :
                          result.type === 'invoice' ? 'bg-orange-100 text-orange-700' :
                          result.type === 'location' ? 'bg-pink-100 text-pink-700' :
                ...
                <div className="flex items-start gap-2 text-xs text-gray-600">
                  ...
                  <div>
                    ...
                    <ul className="space-y-0.5 text-gray-500">
                      ...
                      <li>• Use <kbd className="px-1 py-0.5 bg-white border border-gray-300 rounded text-xs">Ctrl+K</kbd> or <kbd className="px-1 py-0.5 bg-white border border-gray-300 rounded text-xs">Cmd+K</kbd> to open</li>
