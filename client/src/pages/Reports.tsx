...
import { connectionLeads, planUpgrades, supportTickets } from "../services/dataService";

const Reports: React.FC = () => {
  const [dateRange, setDateRange] = useState("6months");

  const analytics = useMemo(() => getCustomerAnalytics(mockBharatNetCustomers), []);
  ...
  const subscriberGrowthData = useMemo(() => {
    ...
    return [
      { month: "May", active: Math.round(total * 0.88), suspended: Math.round(total * 0.08), cancelled: Math.round(total * 0.04) },
      { month: "Jun", active: Math.round(total * 0.90), suspended: Math.round(total * 0.07), cancelled: Math.round(total * 0.03) },
      { month: "Jul", active: Math.round(total * 0.92), suspended: Math.round(total * 0.06), cancelled: Math.round(total * 0.02) },
      { month: "Aug", active: Math.round(total * 0.94), suspended: Math.round(total * 0.05), cancelled: Math.round(total * 0.01) },
      { month: "Sep", active: Math.round(total * 0.96), suspended: Math.round(total * 0.03), cancelled: Math.round(total * 0.01) },
      { month: "Oct", active: analytics.active, suspended: analytics.suspended, cancelled: analytics.cancelled },
    ];
  }, [analytics]);
  ...
  const supportTrendData = useMemo(() => {
    const categories = ["Slow Internet", "No Connectivity", "OTT Issues", "Billing", "Router Issues", "Installation"];
    
    return categories.map((category) => ({
      category: category.replace(" Issues", "").replace(" Query", ""),
      count: supportTickets.filter((t) => t.category === category || t.category === category + " Query").length,
      resolved: supportTickets.filter((t) => (t.category === category || t.category === category + " Query") && t.status === "Resolved").length,
    }));
  }, []);
  ...
  const conversionFunnelData = useMemo(() => {
    const stages = [
      { stage: "New Inquiries", count: connectionLeads.filter(l => l.status === "New").length, percentage: 100 },
      { stage: "Contacted", count: connectionLeads.filter(l => l.status === "Contacted").length, percentage: 75 },
      { stage: "Site Survey", count: connectionLeads.filter(l => l.status === "Site Survey Scheduled").length, percentage: 55 },
      { stage: "Feasibility", count: connectionLeads.filter(l => l.status === "Feasibility Check").length, percentage: 40 },
      { stage: "Quotation", count: connectionLeads.filter(l => l.status === "Quotation Sent").length, percentage: 30 },
      { stage: "Converted", count: connectionLeads.filter(l => l.status === "Converted").length, percentage: 20 },
    ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow-lg p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold mt-2">{analytics.active.toLocaleString()}</p>
        ...
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg shadow-lg p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-purple-100 text-sm font-medium">Network Uptime</p>
        ...
        <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-lg shadow-lg p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold mt-2">{analytics.churnRisk.high}</p>
              <p className="text-orange-100 text-xs mt-1">High risk customers</p>
            </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Monthly recurring revenue over time</p>
          </div>
        </div>
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart data={revenueData}>
            ...
            <YAxis stroke="#6b7280" label={{ value: 'Revenue (₹L)', angle: -90, position: 'insideLeft' }} />
            <Tooltip
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Active, suspended, and cancelled subscribers</p>
          </div>
        </div>
        <ResponsiveContainer width="100%" height={300}>
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Subscriber breakdown by plan type</p>
            </div>
          </div>
          <ResponsiveContainer width="100%" height={300}>
            <RePieChart>
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Customer risk distribution</p>
            </div>
          </div>
          <ResponsiveContainer width="100%" height={300}>
            <RePieChart>
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">Support Ticket Analysis</h3>
            <p className="text-sm text-gray-500 mt-1">Ticket volume and resolution by category</p>
          </div>
        </div>
        <ResponsiveContainer width="100%" height={300}>
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">Connection Conversion Funnel</h3>
            <p className="text-sm text-gray-500 mt-1">Lead progression through sales stages</p>
          </div>
        </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">Plan Upgrade Revenue Impact</h3>
            <p className="text-sm text-gray-500 mt-1">MRR impact by upgrade type</p>
          </div>
        </div>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={upgradeRevenueData}>
            ...
            <YAxis stroke="#6b7280" label={{ value: 'MRR Impact (₹)', angle: -90, position: 'insideLeft' }} />
            <Tooltip
              contentStyle={{ backgroundColor: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px' }}
