...
router.post("/send", requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (customerId || leadId || dealId) {
      const relatedTo = customerId
        ? {
            type: "customer" as const,
            id: new mongoose.Types.ObjectId(customerId),
          }
        : leadId
        ? { type: "lead" as const, id: new mongoose.Types.ObjectId(leadId) }
        : { type: "deal" as const, id: new mongoose.Types.ObjectId(dealId) };
      ...
      await Activity.create({
        ...
        tags: ["email"],
        attachments: [],
        customFields: new Map([["emailId", email._id.toString()]]),
      });
    }
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      await Activity.create({
        ...
        description: `Sent welcome email to ${customer.firstName} ${customer.lastName}`,
        relatedTo: {
          type: "customer",
          id: customer._id,
        },
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (!sent) {
        return res
          .status(500)
          .json({ error: "Failed to send follow-up email" });
      }
      ...
      await Activity.create({
        type: "email",
        subject: `Follow-up email sent: ${subject}`,
        description: `Sent follow-up email to ${recipientName}`,
        relatedTo: customerId
          ? { type: "customer", id: new mongoose.Types.ObjectId(customerId) }
          : { type: "lead", id: new mongoose.Types.ObjectId(leadId) },
        assignedTo: req.user.id,
        createdBy: req.user.id,
        status: "completed",
        priority: "medium",
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const emailPromises = customers.map(async (customer) => {
        ...
        await Activity.create({
          type: "email",
          subject: `Campaign email sent: ${subject}`,
          description: `Sent campaign email to ${customer.firstName} ${customer.lastName}`,
          relatedTo: {
            type: "customer",
            id: customer._id,
          },
...
router.post("/:id/opened", async (req: AuthRequest, res: Response) => {
  try {
    const email = await Email.findById(req.params.id);
